...
export default class VerifyAccount extends Component {
	...
	updatesRenderer_VerifySuccess(ExpiresInHours) {
	...
	onClick_Submit(e) {
		...
		axios.post(APIRoutes.SendAccountVerificationEmail, DATA, { withCredentials: true }).then(res => {
			this.updatesRenderer_VerifySuccess(res.data.verifyaccount.ExpiresInHours);
		}).catch(err => {
			...
			if (err.response) {
				...
				if (err.response.status === 500 || err.response.data.status === 500) {
					...
					this.updatesRenderer_VerifyError();
				} else {
					this.URChildRef.current.addUpdates({ undefined });
					this.setState({
						inputDisabled: false,
